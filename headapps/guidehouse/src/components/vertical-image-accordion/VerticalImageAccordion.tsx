...
export const Default: React.FC<VerticalImageAccordionProps> = ({ fields, isPageEditing }) => {
  ...
  if (fields) {
    if (isEditMode) {
      return (
        <div
          className="@container bg-primary text-primary-foreground relative mx-auto my-6 max-w-7xl px-4 py-16 sm:px-6 lg:px-8"
          role="region"
              ...
              <div key={index} className="flex flex-col overflow-hidden">
                ...
                <div className="flex flex-col py-6">
                  ...
                  {(isEditMode || item?.link?.jsonValue) && (
                    <>
                      {isEditMode ? (
                        <div className="font-heading bg-secondary text-secondary-foreground mt-4 inline-flex w-fit items-center justify-center px-8 py-2.5 text-sm font-medium">
                          <Link
                            field={
                              item.link?.jsonValue || {
                                value: {
